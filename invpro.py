...
def proc(a,d):
    ...
    bill='PWPL/RW/'+d['Date'].strftime("%y")+"/"+d['Date'].strftime("%b")+"/"+d['Roll No.']
    ...
    if os.path.exists(path+'Invoice for roll_no_'+d['Roll No.']+'.pdf'):
        os.remove(path+'Invoice for roll_no_'+d['Roll No.']+'.pdf')
    os.rename(os.getcwd()+'\\Invoice for roll_no_'+d['Roll No.']+'.pdf', path+'Invoice for roll_no_'+d['Roll No.']+'.pdf')
    subprocess.Popen([path+'Invoice for roll_no_'+d['Roll No.']+'.pdf'],shell=True)
    return bill
